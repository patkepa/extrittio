...
pub fn list_legacy_firmware_blobs(
    conn: &mut PgConnection,
    limit: i64,
) -> Result<Vec<FirmwareBlob>, diesel::result::Error> {
    firmware_blobs::table
        .filter(firmware_blobs::storage_backend.eq("database"))
        .filter(firmware_blobs::data.is_not_null())
        .order(firmware_blobs::firmware_update_id.asc())
        .limit(limit)
        .select(FirmwareBlob::as_select())
        .load(conn)
}

pub fn move_firmware_blob_to_object_storage(
    conn: &mut PgConnection,
    tenant_id: &str,
    firmware_update_id: i32,
    storage_backend: &str,
    ...
    .set((
        firmware_blobs::data.eq(None::<Vec<u8>>),
        firmware_blobs::storage_backend.eq(storage_backend),
        firmware_blobs::storage_key.eq(Some(storage_key)),
    ))
    .execute(conn)?;
    Ok(updated > 0)
}
