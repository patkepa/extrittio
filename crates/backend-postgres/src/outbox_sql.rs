...
use crate::schema::rule_action_outbox;
...
pub fn claim_batch(
    ...
    conn.transaction(|conn| {
        ...
        let lease_cutoff = now
            .checked_sub_signed(Duration::microseconds(lease_micros))
        ...
        diesel::sql_query(
            ...
            WITH ranked AS (
                ...
                WHERE outbox.attempts < outbox.max_attempts
                  AND (
                    (outbox.status IN ('pending', 'failed') AND outbox.available_at <= $1)
                    OR (outbox.status = 'processing' AND outbox.locked_at <= $2)
                  )
                ORDER BY ranked.tenant_rank, ranked.available_at, ranked.created_at, ranked.id
                LIMIT $3
            ...
            SELECT claimed.*
            FROM claimed JOIN ranked ON ranked.id = claimed.id
            ORDER BY ranked.tenant_rank, ranked.available_at, ranked.created_at, ranked.id
            "#,
        )
...
pub fn mark_succeeded(
    ...
    diesel::update(
        rule_action_outbox::table
            ...
            .filter(rule_action_outbox::status.eq("processing"))
            .filter(rule_action_outbox::locked_by.eq(claim_token)),
    )
    .set((
...
pub fn mark_failed(
    ...
    diesel::update(
        rule_action_outbox::table
            .filter(rule_action_outbox::id.eq(event_id))
            .filter(rule_action_outbox::status.eq("processing"))
            .filter(rule_action_outbox::attempts.eq(attempts))
            .filter(rule_action_outbox::locked_by.eq(claim_token)),
    )
    .set((
...
pub fn list_dead_letters(
    ...
    rule_action_outbox::table
        ...
        .order((
            rule_action_outbox::updated_at.desc(),
            rule_action_outbox::id.asc(),
        ))
        .limit(limit)
        .offset(offset)
