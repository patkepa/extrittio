CREATE TABLE device_contracts (
  ...
  UNIQUE (tenant_id, id), UNIQUE (tenant_id, device_id, contract_hash),
...
CREATE TABLE device_contract_assignments (
  ...
  FOREIGN KEY (tenant_id, device_id) REFERENCES devices(tenant_id, id) ON DELETE CASCADE,
  FOREIGN KEY (tenant_id, desired_contract_id) REFERENCES device_contracts(tenant_id, id),
  FOREIGN KEY (tenant_id, active_contract_id) REFERENCES device_contracts(tenant_id, id)
);
