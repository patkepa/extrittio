...
pub fn handle_shadow_report(db_pool: &DbPool, topic_device_id: &str, payload: &[u8]) {
    ...
    let Some(identity) =
        super::resolve_ingress_identity(&mut conn, "shadow report", &report.device_id)
    else {
        return;
    };
    ...
    if let Err(e) = shadow_service::update_reported(
        &mut conn,
    ...
    if let Err(e) = shadow_service::process_ota_from_report(
        &mut conn,
        identity.tenant_id_str(),
        identity.device_id(),
        &shadow.reported,
...
pub async fn handle_shadow_get(
    ...
    let shadow = match tokio::task::spawn_blocking(move || {
        ...
        let identity =
            crate::repositories::device_repo::resolve_device_identity(&mut conn, &device_id)
                .map_err(|e| format!("Device identity not found for {device_id}: {e}"))?;
        shadow_service::get_shadow_for_tenant(
            &mut conn,
            identity.tenant_id_str(),
            identity.device_id(),
        )
