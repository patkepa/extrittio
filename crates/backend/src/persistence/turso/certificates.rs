...
fn decode_ca(record: &Row) -> Result<CaCertificateRecord, PersistenceError> {
    Ok(CaCertificateRecord {
        id: row::i32(
            record.get::<i64>(0).map_err(row::error)?,
            "ca_certificates.id",
        )?,
        private_key_pem: record.get(1).map_err(row::error)?,
        certificate_pem: record.get(2).map_err(row::error)?,
...
fn decode_device(record: &Row) -> Result<DeviceCertificateRecord, PersistenceError> {
    Ok(DeviceCertificateRecord {
        ...
        private_key_pem: record.get(2).map_err(row::error)?,
        certificate_pem: record.get(3).map_err(row::error)?,
        fingerprint: record.get(4).map_err(row::error)?,
        expires_at: row::datetime(record.get(5).map_err(row::error)?)?,
...
#[async_trait]
impl CertificateRepository for TursoAdapter {
    async fn get_ca(&self) -> Result<Option<CaCertificateRecord>, PersistenceError> {
        ca_from(&self.database.connect()?).await
    }
    ...
    async fn list_stored_private_keys(
        ...
        let mut ca_rows = connection
            ...
            .await
            .map_err(row::error)?;
        while let Some(record) = ca_rows.next().await.map_err(row::error)? {
            result.push(StoredPrivateKeyRecord::Ca {
                id: row::i32(record.get(0).map_err(row::error)?, "ca_certificates.id")?,
                value: record.get(1).map_err(row::error)?,
            });
        }
        ...
        while let Some(record) = device_rows.next().await.map_err(row::error)? {
            result.push(StoredPrivateKeyRecord::Device {
                tenant_id: record.get(0).map_err(row::error)?,
                id: row::i32(record.get(1).map_err(row::error)?, "device_certificates.id")?,
                value: record.get(2).map_err(row::error)?,
            });
        }
        Ok(result)
    }
