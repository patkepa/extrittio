...
fn decode(record: &Row) -> Result<ApiKeyRecord, PersistenceError> {
    Ok(ApiKeyRecord {
        id: row::i32(record.get::<i64>(0).map_err(row::error)?, "api_keys.id")?,
        name: record.get(1).map_err(row::error)?,
        key_prefix: record.get(2).map_err(row::error)?,
...
#[async_trait]
impl ApiKeyRepository for TursoAdapter {
    async fn create(
        &self,
        tenant: &TenantId,
    ...
    async fn list(&self, tenant: &TenantId) -> Result<Vec<ApiKeySummary>, PersistenceError> {
        ...
        while let Some(record) = rows.next().await.map_err(row::error)? {
            result.push(ApiKeySummary {
                key: decode(&record)?,
                device_type_name: record.get(6).map_err(row::error)?,
            });
        }
        Ok(result)
    }
