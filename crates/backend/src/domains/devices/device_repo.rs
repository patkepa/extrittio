...
/// Resolve the tenant-qualified identity for a device-originated message.
///
/// Device IDs are globally unique in the current protocol and schema. This is
/// the only unscoped lookup that ingestion handlers should perform; every
/// subsequent query must use the returned tenant ID as well as the device ID.
pub fn resolve_device_identity(
    conn: &mut PgConnection,
    id: &str,
) -> Result<DeviceIdentity, diesel::result::Error> {
    let (tenant_id, device_id) = devices::table
        .filter(devices::id.eq(id))
        .select((devices::tenant_id, devices::id))
        .first::<(String, String)>(conn)?;

    DeviceIdentity::new(tenant_id, device_id)
...
) -> Result<Vec<DeviceIdentity>, diesel::result::Error> {
