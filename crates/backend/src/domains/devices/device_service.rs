...
fn hydrate_declared_connections_from_telemetry(
    ...
    let candidate_ids: Vec<String> = devices
        .iter()
        .filter(|(device, _, _)| has_no_declared_connections(&device.declared_connections))
...
fn hydrate_network_observed_hosts(
    ...
    let analyzer_ids: Vec<String> = devices
        .iter()
        .filter(|(device, device_type, _)| is_network_analyzer_device(device, device_type))
        .map(|(device, _, _)| device.id.clone())
        .collect();
...
pub fn auto_register_device(
    ...
    let device_type_id =
        match device_type_repo::find_device_type_by_name(conn, tenant_id, type_name) {
            Ok(Some(dt)) => dt.id,
            Ok(None) => {
                warn!(
                ...
                match device_type_repo::find_default_device_type_id(conn, tenant_id) {
                    Ok(Some(id)) => id,
                    _ => {
...
pub fn mark_devices_offline(
    conn: &mut PgConnection,
    devices_to_mark: &[DeviceIdentity],
) -> Result<usize, AppError> {
    if devices_to_mark.is_empty() {
        return Ok(0);
    }
    ...
    let mut count = 0;
    for identity in devices_to_mark {
        count += diesel::update(
            devices::table.filter(
                devices::tenant_id
                    .eq(identity.tenant_id_str())
                    .and(devices::id.eq(identity.device_id()))
        ...
        if let Err(e) = log_repo::insert_log(
            ...
            tracing::warn!(
                "Failed to insert offline log for {}: {e}",
                identity.device_id()
            );
        }
    }
...
pub fn update_from_heartbeat(
    ...
    let previous_status =
        device_repo::find_device_for_tenant(conn, identity.tenant_id_str(), identity.device_id())
            .ok()
            .map(|d| d.status);
