...
/// Persist a command using its stable outbox delivery ID. Retries may publish
/// the same command again, but they do not create duplicate history rows.
pub fn insert_command_idempotent(
    conn: &mut PgConnection,
    record: &NewCommandRecord,
) -> Result<(), diesel::result::Error> {
    diesel::insert_into(command_history::table)
        .values(record)
        .on_conflict(command_history::id)
        .do_nothing()
        .execute(conn)?;
    Ok(())
}

pub fn find_command_for_tenant(
    conn: &mut PgConnection,
    tenant_id: &str,
    id: &str,
