...
use crate::domains::commands::types::{
    CommandQuery, CommandRecord as PortCommandRecord, NewCommandRecord as PortNewCommandRecord,
};
...
) -> Result<PortCommandRecord, AppError> {
    policy::require(ctx, Permission::SendCommands)?;
...
pub async fn list_commands_with_repository(
    ...
    device_id: &str,
    query: CommandQuery,
) -> Result<Vec<PortCommandRecord>, AppError> {
