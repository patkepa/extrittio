...
pub fn get_shadow_for_tenant(
    conn: &mut PgConnection,
    tenant_id: &str,
    device_id: &str,
) -> Result<DeviceShadow, AppError> {
    Ok(shadow_repo::find_shadow(conn, tenant_id, device_id)?)
}
...
pub fn process_ota_from_report(
    ...
    let deployment_id =
        firmware_repo::find_active_ota_deployment(conn, tenant_id, device_id, fw_id)?;
