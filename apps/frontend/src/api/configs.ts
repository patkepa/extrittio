...
export async function getDeviceConfig(deviceId: string): Promise<DeviceConfigResponse> {
  const { data } = await client.get<DeviceConfigResponse>(`/devices/${deviceId}/config`);
  return data;
}
...
export async function updateDeviceConfig(
  ...
  const { data } = await client.put<DeviceConfigResponse>(`/devices/${deviceId}/config`, config);
  return data;
}
