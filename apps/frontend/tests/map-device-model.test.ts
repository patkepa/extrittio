...
import { locatedMapDevices } from '../src/components/map/map-device-model.ts';
...
test('map locations come only from matching contract observations, including origin', () => {
  ...
  const location = { latitude: 0, longitude: 0, timestamp: '2026-09-15T00:00:00Z', contract_id: 'contract', event_id: 'event' };
  assert.deepEqual(locatedMapDevices(devices, []), []);
  assert.deepEqual(locatedMapDevices(devices, [{ device_id: 'other', location }]), []);
  const result = locatedMapDevices(devices, [{ device_id: 'device', location }]);
  assert.equal(result.length, 1);
  assert.deepEqual(result[0]?.location, location);
  assert.equal(result[0]?.id, 'device');
  for (const invalid of [{ latitude: NaN }, { longitude: Infinity }, { latitude: 91 }, { longitude: -181 }]) {
    assert.deepEqual(locatedMapDevices(devices, [{ device_id: 'device', location: { ...location, ...invalid } }]), []);
  }
});
...
test('location batches deduplicate IDs, cap request size and concurrency, and preserve results', async () => {
  ...
  assert.deepEqual(await loadLocationBatches([], async () => { throw new Error('must not fetch'); }), []);
});

test('a failed location batch rejects the result and stops scheduling further chunks', async () => {
  let calls = 0;
  const ids = Array.from({ length: 2001 }, (_, index) => `device-${index}`);
  await assert.rejects(loadLocationBatches(ids, async () => {
    calls++;
