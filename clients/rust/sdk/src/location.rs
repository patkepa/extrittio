...
impl LocationState {
    ...
    pub fn step(&mut self, lat_offset: f64, lon_offset: f64, speed_offset: f32) {
        ...
        if lat_offset.abs() > f64::EPSILON || lon_offset.abs() > f64::EPSILON {
            let heading_radians = libm::atan2(lon_offset, lat_offset);
            let heading_degrees = heading_radians * 180.0 / core::f64::consts::PI;
            self.heading = (heading_degrees as f32 + 360.0) % 360.0;
        }
