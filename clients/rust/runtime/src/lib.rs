...
pub const EMBED_MARKER_LEN: usize = 23;
pub const EMBED_ID_CAPACITY: usize = 64;
pub const EMBED_SLOT_LEN: usize = 88;

/// Read the device ID from the embedded slot (returns `None` if still blank).
pub fn read_embedded_device_id(embedded_slot: &[u8]) -> Option<String> {
    let id_bytes = embedded_slot.get(EMBED_MARKER_LEN..EMBED_MARKER_LEN + EMBED_ID_CAPACITY)?;
    let end = id_bytes
        .iter()
        .position(|&b| b == 0)
...
pub fn patch_device_id_in_binary(binary: &mut [u8], device_id: &str) -> bool {
...
mod tests {
    ...
    fn blank_slot() -> [u8; EMBED_SLOT_LEN] {
        let mut slot = [0_u8; EMBED_SLOT_LEN];
        slot[..EMBED_MARKER_LEN].copy_from_slice(b"<<EXTRITTIO_DEVICE_ID>>");
        slot
    }

    #[test]
    fn embedded_identity_round_trips_through_firmware_patch() {
        let mut firmware = [
            b"prefix".as_slice(),
            blank_slot().as_slice(),
            b"suffix".as_slice(),
        ...
        .concat();
        assert!(patch_device_id_in_binary(
            &mut firmware,
            "factory-sensor-42"
        ));

        let slot_start = b"prefix".len();
        assert_eq!(
            read_embedded_device_id(&firmware[slot_start..slot_start + EMBED_SLOT_LEN]),
            Some("factory-sensor-42".to_string())
        );
    }

    #[test]
    fn malformed_or_blank_slots_do_not_produce_an_identity() {
        assert_eq!(read_embedded_device_id(&[]), None);
        assert_eq!(read_embedded_device_id(&blank_slot()), None);
    }

    #[test]
    fn firmware_without_marker_is_not_modified() {
        let mut firmware = vec![0_u8; 256];
        assert!(!patch_device_id_in_binary(&mut firmware, "device-1"));
    }
}
