...
Extrittio Edge uses a local-only Turso database for single-node local-device deployments. PostgreSQL remains the default and the only supported production/HA backend. Turso is rejected when `EXTRITTIO_DEPLOYMENT_PROFILE=production`, and database open failures never fall back to another backend.
...
`extrittio run` always selects the local Turso Edge profile. On first run it
...
[OpenThread Edge deployment guide](OPENTHREAD_EDGE.md) covers the nRF52840
...
```bash
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data info
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data integrity
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data checkpoint
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data backup ./backups/extrittio.db
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data verify-backup ./backups/extrittio.db
```
...
```bash
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data \
...
```bash
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data export ./backups/export.json
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data import ./backups/export.json --dry-run
extrittio database --database-backend turso --deployment-profile edge --data-dir ./data import ./backups/export.json
```
...
The CI Edge artifact gate builds Linux amd64 and arm64, runs the Turso unit suite and first-run migration smoke test, and rejects binaries linked to `libpq` or PostgreSQL runtime libraries.
