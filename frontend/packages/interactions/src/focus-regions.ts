...
function getRegionElements(): HTMLElement[] {
  return REGION_ORDER.flatMap((region) =>
    Array.from(document.querySelectorAll<HTMLElement>(`[data-focus-region="${region}"]`)).filter(
      isVisibleElement,
    ),
  );
}
...
function getCurrentRegionIndex(regions: HTMLElement[]): number {
  ...
  const currentRegion = activeElement.closest<HTMLElement>(FOCUS_REGION_SELECTOR);
...
function getFocusTarget(region: HTMLElement): HTMLElement {
  ...
  const focusableTarget = Array.from(region.querySelectorAll<HTMLElement>(FOCUSABLE_SELECTOR)).find(
    (element) => isVisibleElement(element) && !isInNestedRegion(element),
  );
...
export function moveFocusRegion(direction: FocusRegionDirection) {
  ...
  const nextIndex =
    currentIndex === -1
      ? fallbackIndex
      : Math.min(Math.max(currentIndex + (direction === 'right' ? 1 : -1), 0), regions.length - 1);
