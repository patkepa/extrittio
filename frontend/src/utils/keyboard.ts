...
export type DirectionalKey = 'up' | 'down' | 'left' | 'right' | 'first' | 'last';
...
export function hasOpenBlockingOverlay(): boolean {
  return Boolean(
    document.querySelector(
      '.bp6-overlay-open .bp6-dialog, .bp6-overlay-open [role="dialog"], [cmdk-dialog]',
    ),
  );
}

export function getDirectionalKey(event: KeyboardEvent | ReactKeyboardEvent): DirectionalKey | null {
  switch (event.key) {
    case 'ArrowUp':
      return 'up';
    case 'ArrowDown':
