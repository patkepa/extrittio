...
// SAFETY: portable-atomic provides a stable 64-bit atomic abstraction on
// targets where core does not expose AtomicI64.
unsafe impl IStable for portable_atomic::AtomicI64 {
    type Size = U8;
    type Align = expr_to_type!(core::mem::align_of::<Self>());
    type ForbiddenValues = End;
    type UnusedBits = End;
    ...
    #[cfg(feature = "experimental-ctypes")]
    type CType = <Self::Align as Alignment>::AsUint;
    primitive_report!("portable_atomic::AtomicI64");
}
check!(portable_atomic::AtomicI64);
// SAFETY: Automatic checks verify this.
